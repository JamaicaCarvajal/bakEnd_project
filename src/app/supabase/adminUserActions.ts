...
export const addAdminUser = async (user: AdminUser): Promise<void> => {
  ...
  const { error } = await supabase.from('adminuser').insert([payload]);
  if (error) { console.error('Error adding admin user:', error); }
};
...
export const updateAdminUser = async (user: AdminUser): Promise<void> => {
  ...
  const { error } = await supabase.from('adminuser').update(payload).eq('userid', user.userid);
...
export const deleteAdminUser = async (userid: string): Promise<void> => {
  ...
  const supabase = await supabaseServer();
  const { error } = await supabase.from('adminuser').delete().eq('userid', userid);
...
export const getAdminUserByUsername = async (username: string): Promise<AdminUser | null> => {
  const supabase = await supabaseServer();
  const { data, error } = await supabase.from('adminuser').select('*').eq('username', username).maybeSingle();
